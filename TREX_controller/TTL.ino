void TTL()
{
  ...
  if(Serial.available()>2)                                     // Command is 3 bytes in length
  {
    ...
    {
      joyX=0;                                               // bad data received
      joyY=0;                                               // set motor speeds to 0
    ...
    else
    {
      ...
      //rmspeed=(int(Serial.read())-127)*2-1;                    // read values for left and right motors
        joyX=(int)Serial.read();
        joyY=(int)Serial.read();
        JoyToMotor(joyX,joyY);
  }
  }
  else                                                         // less than 3 bytes in buffer
  {
    ...
    {
      joyX=0;                                               // stop left motor
      joyY=0;                                               // stop right motor
...
void TTLConfig()                                         // This code intended for a DAGU bluetooth module - may not work with other brands
{
  Serial.begin(9600);
